...
class InterpretadorRAG:
    ...
    def _load_and_index_documents(self):
        """Cargar y indexar los archivos de interpretaciones"""
        try:
            interpretaciones_dir = Path("data")
            md_files = sorted([f for f in interpretaciones_dir.glob("[0-9]*.md")])
            
            if not md_files:
                raise FileNotFoundError(f"No se encontraron archivos de interpretaciones numerados en: {interpretaciones_dir}")
    ...
    def _load_target_titles(self):
        """Cargar títulos objetivo desde el archivo MD"""
    ...
    async def generar_interpretacion_completa(self, carta_natal_data: Dict[str, Any], genero: str) -> Dict[str, Any]:
        """
        ...
        # Filtrar eventos según títulos objetivo
    ...
    def _generar_consulta_estandarizada(self, evento: Dict[str, Any]) -> str:
        ...
        if tipo == "Aspecto":
            ...
            signo_es = self._translate_sign(evento.get('signo'))
            return f"{angulo_es.lower()} (ángulo) en {signo_es.lower()}"
